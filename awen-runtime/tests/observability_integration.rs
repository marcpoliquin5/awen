...
use awen_runtime::observability::{Level, ObservabilityContext};
...
fn test_complete_observability_flow() {
    ...
    // Top-level run span
    {
        ...
        // IR compilation
        {
            ...
            ctx.metrics
                .counter("engine.operations_executed", 1.0, "operations", {
        ...
        // Engine execution
        {
            ...
            // Simulate HAL operations
            {
                ...
                ctx.metrics
                    .histogram("hal.command_latency_us", 125.5, "microseconds", {
                        let mut attrs = std::collections::HashMap::new();
                        attrs.insert("subsystem".to_string(), "hal".to_string());
                        attrs
                    });
            ...
            // Simulate control operation
            {
                ...
                ctx.events
                    .info("control", "Drift detected, triggering recalibration", {
                        let mut attrs = std::collections::HashMap::new();
                ...
                ctx.metrics
                    .counter("control.calibrations_triggered", 1.0, "events", {
                        let mut attrs = std::collections::HashMap::new();
        ...
        // Storage operation
        {
            ...
            ctx.metrics
                .counter("storage.artifacts_written", 1.0, "artifacts", {
                    let mut attrs = std::collections::HashMap::new();
            ...
            ctx.metrics
                .histogram("storage.artifact_size_bytes", 4096.0, "bytes", {
                    let mut attrs = std::collections::HashMap::new();
                    attrs.insert("subsystem".to_string(), "storage".to_string());
                    attrs
                });
    ...
    ctx.timeline
        .add_entry("Engine", "Run", 0, 1000, Default::default());
    ctx.timeline
        .add_entry("Engine", "IR.compile", 0, 100, Default::default());
    ctx.timeline
        .add_entry("Engine", "Engine.execute", 100, 900, Default::default());
    ...
    ctx.timeline
        .add_entry("Control", "calibrate", 300, 500, Default::default());
    ctx.timeline
        .add_entry("Storage", "write_artifact", 900, 1000, Default::default());
    ...
    assert!(
        artifacts.metadata.exists(),
        "observability_metadata.json should exist"
    );
    ...
    assert!(
        traces_content.contains("IR.compile"),
        "Should contain IR.compile span"
    );
    assert!(
        traces_content.contains("safety_clamp_triggered"),
        "Should contain safety clamp event"
    );
    assert!(
        traces_content.contains("correlation_id"),
        "Should contain correlation IDs"
    );

    // Verify timeline content
    let timeline_content = std::fs::read_to_string(&artifacts.timeline).unwrap();
    assert!(
        timeline_content.contains("Engine"),
        "Should contain Engine lane"
    );
    assert!(
        timeline_content.contains("HAL.Channel.0"),
        "Should contain HAL lane"
    );
    assert!(
        timeline_content.contains("Control"),
        "Should contain Control lane"
    );
    assert!(
        timeline_content.contains("Storage"),
        "Should contain Storage lane"
    );

    // Verify metrics content
    let metrics_content = std::fs::read_to_string(&artifacts.metrics).unwrap();
    assert!(
        metrics_content.contains("engine.operations_executed"),
        "Should contain engine counter"
    );
    assert!(
        metrics_content.contains("hal.safety_clamps"),
        "Should contain HAL counter"
    );
    assert!(
        metrics_content.contains("control.calibrations_triggered"),
        "Should contain control counter"
    );
    assert!(
        metrics_content.contains("hal.command_latency_us"),
        "Should contain HAL histogram"
    );
    assert!(
        metrics_content.contains("storage.artifact_size_bytes"),
        "Should contain storage histogram"
    );

    // Verify events content
    let events_content = std::fs::read_to_string(&artifacts.events).unwrap();
    assert!(
        events_content.contains("warning"),
        "Should contain warning level"
    );
    assert!(events_content.contains("info"), "Should contain info level");
    assert!(
        events_content.contains("Safety clamp applied"),
        "Should contain safety clamp message"
    );
    assert!(
        events_content.contains("Drift detected"),
        "Should contain drift message"
    );

    // Verify metadata
    let metadata_content = std::fs::read_to_string(&artifacts.metadata).unwrap();
    assert!(
        metadata_content.contains("observability.v0.1"),
        "Should contain schema version"
    );
    assert!(
        metadata_content.contains("conformance_level"),
...
fn test_span_hierarchy() {
    ...
    for span in &spans {
        assert!(
            span.attributes.contains_key("correlation_id"),
            "Span {} should have correlation_id",
            span.name
        );
        assert!(
            span.attributes.contains_key("subsystem"),
            "Span {} should have subsystem",
            span.name
        );
    }
}
...
fn test_metrics_types() {
    ...
    // Counter
    ctx.metrics
        .counter("test.counter", 42.0, "operations", Default::default());

    // Gauge
    ctx.metrics
        .gauge("test.gauge", 98.6, "celsius", Default::default());

    // Histogram
    ctx.metrics
        .histogram("test.histogram", 250.5, "milliseconds", Default::default());
...
fn test_event_levels() {
    ...
    ctx.events
        .trace("test", "trace message", Default::default());
    ctx.events
        .debug("test", "debug message", Default::default());
    ctx.events.info("test", "info message", Default::default());
    ctx.events
        .warning("test", "warning message", Default::default());
    ctx.events
        .error("test", "error message", Default::default());
    ctx.events
        .fatal("test", "fatal message", Default::default());
...
fn test_timeline_lanes() {
    ...
    ctx.timeline
        .add_entry(lanes::ENGINE, "operation1", 0, 100, Default::default());
    ctx.timeline
        .add_entry(lanes::SCHEDULER, "schedule", 0, 50, Default::default());
    ctx.timeline.add_entry(
        &lanes::hal_channel(0),
        "configure",
        50,
    ...
    ctx.timeline
        .add_entry(&lanes::hal_channel(1), "apply", 75, 100, Default::default());
    ctx.timeline
        .add_entry(lanes::CONTROL, "calibrate", 100, 200, Default::default());
    ctx.timeline
        .add_entry(lanes::STORAGE, "write", 200, 250, Default::default());
