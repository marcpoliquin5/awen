...
mod tests {
    ...
    fn test_dv_state_preparation_bell() {
        ...
        if let Some(dv_data) = &state.dv_data {
            ...
            for (label, qudit) in &dv_data.qudits {
    ...
    fn test_quantum_state_snapshot() {
        let state = QuantumState::new_cv(vec!["mode_0".to_string(), "mode_1".to_string()], 666, 500);
    ...
    fn test_quantum_artifact_capture() {
        ...
        artifact.intermediate_states.push(state.snapshot().expect("Snapshot failed"));
    ...
    fn test_gaussian_simulator_backend_capabilities() {
        ...
        assert!(supported.iter().any(|b| matches!(b, BasisType::Homodyne { .. })));
        
        // Verify measurement latency
        let latency = backend.measurement_latency();
        assert_eq!(latency.total_latency_ns(), latency.detection_latency_ns() + latency.electronics_latency_ns() + latency.transport_latency_ns());
    }
