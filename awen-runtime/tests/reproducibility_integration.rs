...
use awen_runtime::storage::{
    BundleBuilder, ArtifactType, export_bundle, import_bundle, validate_bundle,
    compute_deterministic_id, short_id, ExportFormat,
};
...
fn test_bundle_export_import_roundtrip() {
    ...
    // Export
    let exported_path = export_bundle(&original_bundle, temp_dir.path(), ExportFormat::Directory).unwrap();
    ...
    assert_eq!(original_bundle.parameters_initial, imported_bundle.parameters_initial);
}
...
fn test_checksum_validation() {
    ...
    // Import should fail
    let result = import_bundle(&exported_path);
    assert!(result.is_err(), "Checksum validation should fail on corrupted file");
}
