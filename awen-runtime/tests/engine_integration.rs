...
mod engine_tests {
    ...
    fn create_graph_with_nodes(node_ids: Vec<&str>) -> crate::engine_v2::ComputationGraph {
        let nodes = node_ids
            .iter()
            .map(|id| crate::engine_v2::ComputationNode {
                id: id.to_string(),
    ...
    fn test_phase_shifter_node_execution() {
        ...
        graph.nodes[0].node_type = crate::engine_v2::NodeType::ClassicalPhotonic {
    ...
    fn test_quantum_gate_execution() {
        ...
        graph.nodes[0].node_type = crate::engine_v2::NodeType::QuantumGate {
    ...
    fn test_measurement_in_computational_basis() {
        ...
        graph.nodes[0].node_type = crate::engine_v2::NodeType::Measurement {
    ...
    fn test_multiple_measurements() {
        let engine = crate::engine_v2::Engine::new();
        let graph = create_graph_with_nodes(vec!["m0", "m1", "m2"]);
    ...
    fn test_calibration_node_execution() {
        ...
        graph.nodes[0].node_type = crate::engine_v2::NodeType::Calibration;
    ...
    fn test_coherence_violation_on_deadline_exceeded() {
        ...
        match result {
            Ok(res) => {
                ...
                assert!(res.coherence_violations > 0 || res.status != crate::engine_v2::ExecutionStatus::Success);
            }
            Err(_) => {
    ...
    fn test_quantum_nodes_consume_coherence_budget() {
        ...
        let graph = {
            ...
            g.nodes[0].node_type = crate::engine_v2::NodeType::QuantumGate {
                gate_name: "Hadamard".to_string(),
            };
            g.nodes[0].timing_contract.coherence_requirement_ns = Some(1_000_000);  // 1ms
            
            g.nodes[1].node_type = crate::engine_v2::NodeType::QuantumGate {
                gate_name: "CNOT".to_string(),
            };
    ...
    fn test_same_seed_produces_same_results() {
        ...
        let result1 = engine.run_graph(&graph1, Some(12345)).expect("First run failed");
        let result2 = engine.run_graph(&graph2, Some(12345)).expect("Second run failed");
    ...
    fn test_different_seeds_may_differ() {
        ...
        let result1 = engine.run_graph(&graph1, Some(111)).expect("First run failed");
        let result2 = engine.run_graph(&graph2, Some(222)).expect("Second run failed");
    ...
    fn test_replay_with_measurement_outcomes() {
        ...
        graph1.nodes[0].node_type = crate::engine_v2::NodeType::Measurement {
            basis_type: "Computational".to_string(),
        };
        graph1.nodes[1].node_type = crate::engine_v2::NodeType::Measurement {
            basis_type: "Computational".to_string(),
        };

        let result1 = engine.run_graph(&graph1, Some(99999)).expect("First run failed");

        let mut graph2 = create_graph_with_nodes(vec!["m0", "m1"]);
        graph2.nodes[0].node_type = crate::engine_v2::NodeType::Measurement {
            basis_type: "Computational".to_string(),
        };
        graph2.nodes[1].node_type = crate::engine_v2::NodeType::Measurement {
            basis_type: "Computational".to_string(),
        };

        let result2 = engine.run_graph(&graph2, Some(99999)).expect("Second run failed");
    ...
    fn test_invalid_edge_target_node_error() {
        ...
        graph.edges.push(crate::engine_v2::Edge {
    ...
    fn test_branching_graph_execution() {
        let engine = crate::engine_v2::Engine::new();
        let mut graph = crate::engine_v2::ComputationGraph {
            graph_id: Uuid::new_v4().to_string(),
            nodes: vec![
    ...
    fn test_empty_graph_execution() {
        let engine = crate::engine_v2::Engine::new();
        let graph = crate::engine_v2::ComputationGraph {
            graph_id: Uuid::new_v4().to_string(),
            nodes: vec![],
            edges: vec![],
            root_nodes: vec![],
    ...
    fn test_node_failure_tracking() {
        ...
        match result {
            Ok(res) => {
                ...
                assert!(res.nodes_failed > 0 || res.status != crate::engine_v2::ExecutionStatus::Success);
            }
            Err(_) => {
                // Error caught
            }
