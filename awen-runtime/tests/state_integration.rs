...
use awen_runtime::state::{
    QuantumState, QuantumMode, CoherenceWindow,
    StateEvolver, CoherenceManager,
    ReferenceStateEvolver, ReferenceCoherenceManager,
    MemoryPrimitive, DelayBuffer, ResonatorStore, HybridRegister,
};
...
fn test_unitary_evolution_deterministic() {
    ...
    let coherence_window = CoherenceWindow {
        ...
        start_ns: 0,
        end_ns: 10_000,
        decoherence_timescale_ns: Some(5000.0),
        cross_mode_decoherence_ns: Some(7500.0),
        idle_time_budget_ns: Some(1000),
        notes: None,
    };
...
fn test_resonator_exponential_decay() {
    ...
    // Check approximate decay (within 20% for numerical errors)
    let expected_ratio = (-1.0_f64).exp();
    let actual_ratio = amp_2 / amp_1;
    assert!((actual_ratio - expected_ratio).abs() / expected_ratio < 0.2);
}
...
fn test_state_evolution_sequence() {
    ...
    let state_1 = evolver.evolve_state(&initial_state, "PS", &ps_params).unwrap();
    
    let mut sqz_params = HashMap::new();
    sqz_params.insert("mode_id".to_string(), 0.0);
    sqz_params.insert("r".to_string(), 0.2);
    
    let state_2 = evolver.evolve_state(&state_1, "SQUEEZING", &sqz_params).unwrap();
    
    // Check provenance tracks both operations
    assert!(state_2.provenance.contains_key("last_gate"));
    assert!(state_2.provenance.get("last_gate").unwrap().contains("SQUEEZING"));
}
