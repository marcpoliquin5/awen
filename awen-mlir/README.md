...
The implemented GEMM pipeline is:
...
The v1 importer deliberately supports only rank-two `stablehlo.dot_general`
GEMM. A producer normalization step must materialize the dimension-number
attribute as these four dense i64-array operation attributes:
...
Batching arrays must be empty; contracting dimensions must be lhs `[1]` and
rhs `[0]`. F16, BF16, F32, and complex floating-point element types are
preserved. Static and dynamic result dimensions, optional
